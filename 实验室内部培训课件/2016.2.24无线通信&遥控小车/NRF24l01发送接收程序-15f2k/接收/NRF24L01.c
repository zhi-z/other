...
//#define Tx_Address_Width    5   // 要发送的地址的宽度为5字节，要于接收模块对应
//#define Rx_Address_Width    5   // 要接收的地址的宽度为5字节，要于发送模块对应
//uint const Rx_ADDRESS[Tx_Address_Width]= {0x11,0x22,0x33,0x44,0x55};//本地地址，5字节
//uint const Tx_Address[Rx_Address_Width]= {0xAA,0xBB,0xCC,0xDD,0xEE};//接收地址，5字节

#define Tx_Address_Width    1   // 要发送的地址的宽度为5字节，要于接收模块对应
#define Rx_Address_Width    1   // 要接收的地址的宽度为5字节，要于发送模块对应
uint const Rx_ADDRESS[Tx_Address_Width]= {0x11};//本地地址，5字节
uint const Tx_Address[Rx_Address_Width]= {0xAA};//接收地址，5字节
...
uint SPI_ReadWrite(uint dat)
{
	uint bit_ctr;
   	...
   	{
		MOSI = (dat & 0x80);    //将dat的高位输出MOSI   1000 0000
...
void SetRX_Mode(void)
{
	...
	CE = 1; 								  //WRITE_REG：写寄存器指令；CONFIG：配置收发状态 
	Delay_us(130);
}
